...
class MultiCNT:
    ...
    def calculate_torques(self, cnt_idx, timestep):
        ...
        # Calculate T-fr
        theta_dot = self.dx_dt(self.theta[0:timestep-1, cnt_idx])[-1]
        r_e = a**(1/3)*b**(2/3)
        p = a/b
    ...
    def calculate_forces(self, cnt_idx, timestep):
        ...
        p = a/b
        K_t = (np.sqrt(1 - p ** (-2))) / (p ** (-2 / 3) * np.log(p * (1 + np.sqrt(1 - p ** (-2)))))
        x_dot = self.dx_dt(self.x[0:timestep-1, cnt_idx])[-1] 
        y_dot = self.dx_dt(self.y[0:timestep-1, cnt_idx])[-1]
        F_fr_x = 6*np.pi*self.get_viscosity(timestep)*r_e*K_t*x_dot
        F_fr_y = 6*np.pi*self.get_viscosity(timestep)*r_e*K_t*y_dot
        ...
        for idx in range(self.num_cnts):
            if idx != cnt_idx:
                ...
                F_plus_plus_x = forces[0][0] * np.cos(forces[0][1] + self.theta[timestep - 1, cnt_idx])
                F_plus_plus_y = forces[0][0] * np.sin(forces[0][1] + self.theta[timestep - 1, cnt_idx])
                F_plus_minus_x = forces[1][0] * np.cos(forces[1][1] + self.theta[timestep - 1, cnt_idx])
                F_plus_minus_y = forces[1][0] * np.sin(forces[1][1] + self.theta[timestep - 1, cnt_idx])
                F_minus_minus_x = forces[2][0] * np.cos(forces[2][1] + self.theta[timestep - 1, cnt_idx])
                F_minus_minus_y = forces[2][0] * np.sin(forces[2][1] + self.theta[timestep - 1, cnt_idx])
                F_minus_plus_x = forces[3][0] * np.sin(forces[3][1] + self.theta[timestep - 1, cnt_idx])
                F_minus_plus_y = forces[3][0] * np.sin(forces[3][1] + self.theta[timestep - 1, cnt_idx])
                Forces_on_i_x.append(F_plus_plus_x + F_plus_minus_x + F_minus_minus_x + F_minus_plus_x)
                Forces_on_i_y.append(F_plus_plus_y + F_plus_minus_y + F_minus_minus_y + F_minus_plus_y)
        F_coup_x = sum(Forces_on_i_x)
